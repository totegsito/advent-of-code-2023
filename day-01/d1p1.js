...
// Solution
const resolve = (input) => {
    const firstDigitRegEx = /^[a-z]*(\d)/;
    const lastDigitRegEx = /(\d)[a-z]*$/;
    const numbers = input.map((str) => {
        const firstDigit = str.match(firstDigitRegEx)[1];
        const lastDigit = str.match(lastDigitRegEx)[1];
...
console.assert(test === 142, 'Test case failed');
