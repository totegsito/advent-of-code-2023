// Personal data set
const inputCase = `fouronevhnrz44
eightg1
...
// Solution
const resolve = (input) => {
    const firstDigitRegEx = /(\d|(?:one)|(?:two)|(?:three)|(?:four)|(?:five)|(?:six)|(?:seven)|(?:eight)|(?:nine)).*$/;
    const lastDigitRegEx = /^.*(\d|(?:one)|(?:two)|(?:three)|(?:four)|(?:five)|(?:six)|(?:seven)|(?:eight)|(?:nine))/;
    const numbers = input.map((str) => {
        let firstDigit = str.match(firstDigitRegEx)[1];
        let lastDigit = str.match(lastDigitRegEx)[1];
        if (numberMap[firstDigit]) firstDigit = numberMap[firstDigit];
        if (numberMap[lastDigit]) lastDigit = numberMap[lastDigit];
...
console.assert(test === 281, 'Test case failed');
